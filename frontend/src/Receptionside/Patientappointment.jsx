...
function Patientappointment() {
    ...
    const handleUndoAppointment = async (id) => {
        try {
            const response = await fetch(`http://localhost:8082/api/patient/undo/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            if (response.status) {
                const updatedActiveStatus = { ...activeStatus, [id]: true };
                setActiveStatus(updatedActiveStatus);
                alert("Appointment undone successfully");
            } else {
                console.error("Failed to undo appointment");
            }
        } catch (error) {
            console.error("Error undoing appointment:", error);
        }
    };
        ...
        <>
            ...
            <div className="tab-rece">
                <table>
                    ...
                    <tbody>
                        {filteredPatients?.filter((item => item.raddress === user.data.id)).map(patient => (
                            <tr key={patient.id}>
                                <td>{patient.id}</td>
                                <td>{patient.name}</td>
                                <td>{patient.sex}</td>
                                ...
                                <td>
                                    ...
                                    >
                                        {activeStatus[patient.id] ? "Mark Appointment Done" : "Appointment Done"}
                                    </button>
                                    <button
                                        style={{
                                            color: "black",
                                        ...
                                        onClick={() => handleUndoAppointment(patient.id)}
                                        disabled={activeStatus[patient.id]} // Disable if appointment is not done
                                    >
                                        Undo
                                    </button>
